...
int SH1106Ui::update(){
  ...
  if ( timeBudget <= 0) {
    // Implement frame skipping to ensure within time budget
...
void SH1106Ui::forceUpdate() {
    int tmp = ticksPerTransition;
    ticksPerTransition = 0;
    this->tick();
    this->dirty = false;
    this->display->clear();
    this->drawIndicator();
    ...
    this->drawOverlays();
    this->display->display();
    ticksPerTransition = tmp;
}

void SH1106Ui::drawBitmap(int x, int y, int width, int height, const char *bitmap) { 
  this->display->drawBitmap(x, y, width, height, bitmap);
}
...
//tnis steps outside the frame paradigm - remove, refactor, or live with it
void SH1106Ui::render() {
